        ...
        <div class="wx_wrap" id="detailCon">
            ...
            <div class="mod_btns">  
                <?php
                if ($order->status == 2) {
                    echo CHtml::link('确认收货', $this->createUrl('/m/orders/confirm', array('sn' => $order->sn)), array('class' => 'mod_btn bg_1'));
                    echo CHtml::link('申请退款', $this->createUrl('refund/order', array('sn' => $order->sn)), array('class' => 'mod_btn bg_1'));
                } elseif ($order->status == 3) {
                    echo CHtml::link('确认收货', $this->createUrl('/m/orders/confirm', array('sn' => $order->sn)), array('class' => 'mod_btn bg_1'));
                } else if ($order->is_pay == 0) {
                    echo CHtml::link('付款', $this->createUrl('/m/order/pay', array('sn' => $order->sn)), array('class' => 'mod_btn bg_1'));
                }
                ?>
                <a class="mod_btn bg_2 " href="<?php echo $this->createUrl('/m/order/remove', array('sn' => $order->sn)); ?>">申请取消订单</a>    
            </div>  
            <div class="address">  
                ...
                <?php foreach ($order['OrderItem'] as $k => $item): ?>
                    ...
                    <div class="goods_bd">  
                        <div class="goods_item">  
                            <div class="goods_img">
                                <a href="<?php echo $this->createUrl('/m/item/detail', array('item_id' => $item->item_id)); ?>">
                                    <img src="<?php echo $item->pic; ?>" loaded="1">
                                </a>
                            </div> 
                            <div class="goods_name">
                                <a href="<?php echo $this->createUrl('/m/item/detail', array('item_id' => $item->item_id)); ?>"><?php echo $item->title; ?> </a>
                            </div> 
