            ...
            <div class="refund_detail">
                <script type="text/javascript">
                    var updateStatus = function(status) {
                        var url = "<?php echo $this->createUrl('detail') . '?sn=' . $model->refund_sn; ?>";
                        window.location = url + '&status=' + status;
                    }
                </script>
                <h2><?php echo $model->statusOptions(); ?></h2>
                <?php
                    switch ($model->status)
                    {
                        case 1:
                            echo CHtml::button('取消退款申请', array('onclick' => "updateStatus(5)", 'class' => 'btn-hot'));
                            break;

                        case 2:
                            echo CHtml::button('重新申请', array('onclick' => "updateStatus(1)", 'class' => 'btn-hot'));
                            break;

                        case 3:
                            echo CHtml::button('已退货', array('onclick' => "updateStatus(4)", 'class' => 'btn-hot'));
                            break;
                    }
                ?>
            </div>
