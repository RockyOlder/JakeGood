...
/**
 * Description of CurdController
 *
 * @author Fighter
 */
class CurdController extends SellerController {
    ...
    public function actionSave($ret = FALSE)
    {
        ...
        {
            return parent::renderError('保存出错', CHtml::errorSummary($model));
    ...
    protected function full_form_columns($col, $model = NULL)
    {
        ...
        {
            //当是新增记录时 默认值为空
            $data[$v]['field'] = CHtml::activeTextField($model, $v, array('id' => '_' . $v, 'class' => 'form-control'));
        }

        $data[$this->pk]['field'] = CHtml::activeTextField($model, $this->pk, array('id' => '_' . $v, 'class' => 'form-control', 'readonly' => 'readonly'));
    ...
    public static function catName($col, $data)
    {
        return ItemCats::model()->findByPk($data->$col)->name;
    }
        
    public static function areaName($col, $data)
    {
        return Area::model()->findByPk($data->$col)->name;
    }
    
    public static function dateFormat($col, $data)
    {
        return date('Y-m-d', $data->$col);
    }
