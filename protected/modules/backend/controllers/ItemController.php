...
class ItemController extends CurdController {
            ...
            if (is_array($vid))
            {
                foreach ($vid as $v)
                {
                    ...
                    if ($propValue !== NULL)
                    {
                        ...
                        {
                            $spec[] = array('cid' => $cid, 'pid' => $pid, 'vid' => $vid, 'pname' => $itemProp->name, 'vname' => $propValue->name, 'input' => '');
                        }
                    }
                }
            }
    ...
    {
        $cid      = $this->request->getQuery('cid', 0);
        ...
        $codition = 'parent_id='.$parentId.' ';
        if ($cat && $cat->parent_cid == 0)
        {
            $codition .= 'AND cid = '.$cat->cid;
        }
        elseif ($cat && $cat->parent_cid > 0)
        {
            $codition .= 'AND cid in ('.$cat->cid.','.$cat->parent_cid.') ';
        }
        ...
        if ($parentId == 0)
        {
            ...
            echo CHtml::activeDropDownList($item, 'shop_id', $list, array('class' => 'form-control input-medium'));
        }
        else
        {
            ...
            echo CHtml::checkBoxList('shops', $shops, CHtml::listData($models, 'id', 'name'));
        }
        die;
    }
}
