$(function () {
    ...
    list_menu.on('change', function(){
        ...
        if (($.cookie('header') == 'header-fixed') && ($(this).val() == 'sidebar-icons')) {
            alert('Hi! This option is good, but let try menu option "style 4", it\'s better');
            return;
        }
    ...
    $('#chat-form .chat-group a').unbind('*').click(function(){
        ...
        $('#chat-form .chat-group a').removeClass('active');
        $(this).addClass('active');
        $('#tojid').val($(this).attr('number'));
		$('.chat-box-body').hide();
		$('#his'+$(this).attr('number')).show();
		$('#u'+$(this).attr('number')+' .badge').html('');
        ...
        $('.chat-content > .chat-box-bodys').slimScroll({
            "height": "250px",
            'width': '340px',
            "wheelStep": 5,
    ...
    $('.chat-textarea input').on("keypress", function(e){
        ...
        var $your_avt = 'http://i.anarry.com/avatar?u='+$('#tojid').val();
        if (e.which == 13) {
            var $content = $obj.val();
            ...
            if ($content !== "") {
                ...
                var m = d.getMinutes();
                if (m < 10) m = "0" + m;
				sendmsg($content);	//call func sendmsg send message to friend
                $obj.val(""); // CLEAR TEXT ON TEXTAREA
				/*
                var $element = ""; 
                $element += "<li>";
                $element += "<p style='text-align:right'>";
                $element += "<span class='time'>" + h + ":" + m + "</span>";
                $element += "<img class='avt' src='"+$my_avt+"'>";
                //$element += "<span class='user'>John Doe</span>";
                $element += "</p>";
                $element = $element + "<p style='text-align:right'>" + $content +  "</p>";
                $element += "</li>";
