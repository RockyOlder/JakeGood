    ...
    function buildSkuTable( array) {
	  ...
	  for (var k = 0; k < a.length; k++) {
		  ...
		  for (var i = 0; i < count; i++) {
			body += '<td><input data-index="'+i+'" data-id="'+idArray[k].slice(0,i+1).join(';') +'" value="' +  arr[i][0] + '" type="hidden" name="Item[skus][table]['+k+'][props]['+arr[i][2]+']">' +  arr[i][1] + '</td>';
		  }
		  var sku_row_id = idArray[k].join(';');
		  body += '<td><input type="hidden" class="skusid" data-props="'+sku_row_id +'" value="0" name="Item[skus][table]['+k+'][sku_id]"><input label="price" size="10" class="skus-price" value="" type="text" data-id="'+sku_row_id +'" name="Item[skus][table]['+k+'][price]"></td><td><input label="stock" size="10" class="skus-qty" data-id="'+sku_row_id +'" value="" type="text" name="Item[skus][table]['+k+'][stock]"></td><td><input label="outer_id" class="skus-outer_id" value="" data-id="'+sku_row_id +'" type="text" name="Item[skus][table]['+k+'][outer_id]"></td><td class="operation"><a href="javascript:void(0);" data-id="'+sku_row_id+'" ><i class="icon-cog"></i></a></td></tr>';
	  }
	...
	*/
	...
	function renderTable(){
		...
		$(this).find('.change').filter(':checked').each(function() {
		    newArray.push($(this).val()+";"+$('[for="'+$(this).attr('id')+'"]').text()+";"+$(this).data('id'));
		});
		array.push(newArray);
	    });
	    buildSkuTable(array);
